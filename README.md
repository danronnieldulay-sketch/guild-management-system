...
### 0. **Install Composer (First Time Only):**
If you don't have Composer installed, download and install it:

**Windows:**
1. Download from: https://getcomposer.org/download/
2. Run the Windows installer (Composer-Setup.exe)
3. Follow the installation wizard
4. During setup, point to your PHP installation:
...
### 0b. **Understanding Our Packages (composer.json):**
All PHP packages are defined in `composer.json` at the root of the project. When you run `composer install`, it reads this file and downloads all required packages into the `vendor/` folder.

**Our packages include:**
- `firebase/php-jwt` - For JWT token authentication
- `firebase/php-sdk` - For Firebase integration
- `php-dotenv` - For environment variable management
- Other utilities for database management and API requests

The `composer.json` file specifies exact versions, ensuring consistent package installations across different environments (local, testing, production).

### 2. **Clone/Download to XAMPP:**
```bash
cd C:\xampp\htdocs
...
### 6. **Setup Firebase (Optional for Chat):**
