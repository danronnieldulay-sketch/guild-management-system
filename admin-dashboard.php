        ...
        class GuildAdminDashboard {
            ...
            renderMembers() {
                ...
                this.members.forEach(m => {
                    let statusDisplay;
                    if (m.status === 'Offline' && m.last_active) {
                        const minutes = Math.floor((Date.now() - new Date(m.last_active).getTime()) / 60000);
                        let timeAgo;
                        if (minutes < 60) {
                            timeAgo = `${minutes} min ago`;
                        } else if (minutes < 1440) {
                            timeAgo = `${Math.round(minutes / 60)} hours ago`;
                        } else {
                            timeAgo = `${Math.round(minutes / 1440)} days ago`;
                        }
                        statusDisplay = `Offline (${timeAgo})`;
                    } else {
                        statusDisplay = m.status || 'Offline';
                    }
            ...
            renderMemberRoles() {
                ...
                this.roles.forEach(role => {
                    ...
                    roleCard.innerHTML = `
                        ...
                        <button type="button" class="member-role-equip-btn ${isEquipped ? 'unequip-btn' : ''}" onclick="dashboard.toggleMemberRole(${roleId}, '${roleName.replace(/'/g, "\\'")}', ${isEquipped ? 'true' : 'false'})">
                            <svg class="icon-xs" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ${isEquipped ? '<path d="M6 18L18 6M6 6l12 12"/>' : '<path d="M12 5v14M5 12h14"/>'}
                            </svg>
                            ${isEquipped ? 'Unequip' : 'Equip'}
                        </button>
                    `;
