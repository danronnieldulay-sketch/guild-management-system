...
class Guild {
    ...
    public function createGuild($leaderId, $guildName, $game, $leaderIGN, $leaderUsername, $description, $requirements) {
        ...
        $memberInsert = $this->pdo->prepare("INSERT INTO members (guild_id, user_id, ign, role, authority, status, joined_at, last_active) VALUES (?, ?, ?, 'leader', 'Leader', 'Online', NOW(), NOW())");
    ...
    public function approveApplication($applicationId, $actorUserId) {
        ...
        // Check if user is currently online in any of their existing guild memberships
        $onlineCheck = $this->pdo->prepare("SELECT status FROM members WHERE user_id = ? LIMIT 1");
        $onlineCheck->execute([$application['user_id']]);
        $userStatus = $onlineCheck->fetch(PDO::FETCH_ASSOC);

        // If user has existing memberships, check if they're currently online
        // Otherwise they're offline (brand new user)
        $status = ($userStatus && $userStatus['status'] === 'Online') ? 'Online' : 'Offline';

        // Always use NOW() for new guild join - each guild starts fresh
        $insert = $this->pdo->prepare("INSERT INTO members (guild_id, user_id, ign, role, authority, status, joined_at, last_active) VALUES (?, ?, ?, 'None', 'member', ?, NOW(), NOW())");
        $insert->execute([$application['guild_id'], $application['user_id'], $application['ign'], $status]);
